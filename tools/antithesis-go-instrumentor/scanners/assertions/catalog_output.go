...
type GenInfo struct {
	...
	// the package the catalog is generated into
	PackageName         string
...
func IsGeneratedFile(file_name string) bool {
	...
	return base_name == common.GENERATED_CATALOG_FILE
}
...
// GenerateAssertionsCatalog renders the assertion catalog into the given
// output directory as antithesis_catalog.go.
func GenerateAssertionsCatalog(outputDir string, genInfo *GenInfo) error {
	if genInfo.PackageName == "" {
		return fmt.Errorf("cannot generate an assertion catalog with no package name")
	}
	...
	return tmpl.Execute(outFile, genInfo)
}

func getExpectorText() string {
	const text = `// Code generated by antithesis-go-instrumentor; DO NOT EDIT.
...
{{if usesConst .ConstMap "reachabilityTest"}}  const reachabilityTest = "reachability" {{- end}}
  ...
  // {{$assertionName}}
  assert.AssertRaw({{$cond}}, {{$message}}, noDetails, {{$classname}}, {{$funcname}}, {{$filename}}, {{.Line}}, 0, {{$didHit}}, {{$mustHit}}, {{$assertType}}, {{$displayname}}, {{$message}})
	{{- end}}
}
{{- end}}
`
