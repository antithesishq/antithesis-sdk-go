...
func ParseExclusionsFile(path string, inputDirectory string) (exclusions map[string]bool, err error) {
	...
	logger.Printf(common.Normal, "Reading exclusions from %s; relative paths will be resolved to %s", path, inputDirectory)
	...
	for scanner.Scan() {
		...
		if _, err = os.Stat(exclusion); err == nil {
			...
			logger.Printf(common.Normal, "Exclusion %s added as %s", entry, exclusion)
		} else {
